...
public class FilesWriter {
	...
	public void writeFilesTraining(String nameProj, List<VersionCommits> versionsList, List<JavaClass> classList, int actualRel) {
        int i = 0;
		...
		try(FileWriter writerCSV = new FileWriter(csvFilePath)) {
	       ...
	       writerCSV.append("Release;Class Name;Size;nR;nAuth;LOC Touched;LOC Added;Max LOC Added;Avg LOC Added;Churn;Max Churn;Avg Churn;Is Buggy");
	       writerCSV.append("\n");
        ...
        try (FileWriter arffWriter = new FileWriter(arffFilePath)) {
            ...
            for(i=0;i<actualRel; i++) {
            	...
            	for(JavaClass classJ: classList) {
            		if(classJ.getVersion() == trainVer.getVersion()) { //VEDI SE LASCIARE COSI' O METTENDO INDEX CAMBIA, AMCHE NEGLI ALTRI
	...
	public void writeFilesTesting(String nameProj, List<VersionCommits> versionsList, List<JavaClass> classList, int actualRel) {
        ...
        try (FileWriter arffWriter = new FileWriter(arffFilePath)) {
        	...
        	VersionCommits testVer = versionsList.get(actualRel);
        	for(JavaClass classJ: classList) {
        		if(classJ.getVersion() == testVer.getVersion()) { //VEDI SE LASCIARE COSI' O METTENDO INDEX CAMBIA, AMCHE NEGLI ALTRI
        			writeDataInArff(arffWriter, classJ);
        		}
        	}            
	...
	private void writeDataInCSV(FileWriter writer, VersionCommits vers, List<JavaClass> classList) throws IOException {
		...
		for(JavaClass jClass: classList) {
	    	if(jClass.getVersion() == vers.getVersion()) { //VEDI SE LASCIARE COSI' O METTENDO INDEX CAMBIA, AMCHE NEGLI ALTRI
	...
	private void writeDataInArff(FileWriter writer, JavaClass jClass) throws IOException {
		...
		writer.append(size).append(",").append(nR).append(",")
        .append(nAuth).append(",").append(touchLoc).append(",")
        .append(addedLoc).append(",").append(maxAddedLoc).append(",")
        .append(avgAddedLoc).append(",").append(churn).append(",")
        .append(maxChurn).append(",").append(avgChurn).append(",").append(buggy);
	...
	public static void writeFinalCSV(List<List<ClassifierData>> allValueWeka, String nameProj, int iterations) {
		...
		try (FileWriter writerCSV = new FileWriter(csvFilePath);) {
			 ...
			 writerCSV.append("Project;N° Iteration;Classifier;Feature Selection;Sampling;Cost Sensitive;Training %;Precision;Recall;AUC;Kappa;True Positive;False Positive;True Negative;False Negative");
		     writerCSV.append("\n");			 
