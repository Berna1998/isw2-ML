...
public class MetricsCalculator {
	...
	public void calculateNFix(Ticket tick, JavaClass jClass) { //POI VEDI SE METTERLO
		...
		for(RevCommit comm: commLis) {
			...
			if(mex.contains(key) && comm.getAuthorIdent().getWhen().compareTo(tick.getFv().getDate())<=0 && comm.getAuthorIdent().getWhen().compareTo(tick.getIv().getDate())>0) {
				nFix++;
			}
		}
	}
	...
	public void calculateLOCAndChurnMetrics(JavaClass jClass) {
		//TOuched LOC, Added Loc, MAX Added LOC, AVG Added LOC, Churn, Max Churn e Avg Churn
		int touchLOC = 0;
		int addLOC = 0;
		int maxLOC = 0;
		double avgAddLOC = 0;
